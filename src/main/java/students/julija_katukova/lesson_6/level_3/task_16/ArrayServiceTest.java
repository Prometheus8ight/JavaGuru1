...
class ArrayServiceTest {
    ...
    public void positiveNumbers1 (int[] arr) {
        ...
        check(arr, new int[]{3, 2, 1}, "Positive Numbers Test1");
    }

    public void positiveNumbers2 (int[] arr) {
        ArrayService arrayService = new ArrayService();
        arrayService.revert(arr);
        check(arr, new int[]{4000, 3000, 2000, 1000}, "Positive Numbers Test2");
    }

    public void zeroNumber1 (int[] arr) {
        ArrayService arrayService = new ArrayService();
        arrayService.revert(arr);
        check(arr, new int[]{0, 0, 1, 0, 1, 0}, "Zero Number Test1");
    }

    public void negativeNumbers1 (int[] arr) {
        ArrayService arrayService = new ArrayService();
        arrayService.revert(arr);
        check(arr, new int[]{-3, -89856151, -2, -5696511, -1}, "Negative Numbers Test1");
    }

    public void mixNumbers1 (int[] arr) {
        ArrayService arrayService = new ArrayService();
        arrayService.revert(arr);
        check(arr, new int[]{0, 942587, -2, -87981, 1}, "Mix Numbers Test1");
    }
